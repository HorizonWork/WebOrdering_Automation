...
Danh sách các việc cần làm / nâng cấp quan trọng để đưa agent về đúng kiến trúc **Planner + Controller + Observer + Executor**, chạy ổn định trên 2 sàn TMDT VN và an toàn hơn với người dùng.

---

## P0 - Ổn định code hiện tại (blocking issues)

- [x] **Sửa lỗi cú pháp / encoding trong code Python**
  - Đã rà các file chính: `src/orchestrator/agent_orchestrator.py`, `src/planning/react_engine.py`, `src/execution/browser_manager.py`, `src/orchestrator/safety_guardrails.py`, v.v. để đảm bảo:
    - Không còn ký tự lạ trong string log (kiểu `"�o"`, `"dY"`, …) gây hỏng cú pháp.
    - Không còn lệnh log sai cú pháp kiểu `logger.info(f"dY", Step {step}/{self.max_steps}")` – luôn dùng 1 string format hợp lệ (`logger.info("Step %d/%d", step, self.max_steps)` hoặc f-string đúng chuẩn).
  - Đã chạy lại import + `py_compile` cho các module quan trọng để chắc chắn code Python compile được.

- [x] **Đồng bộ `run_agent.py` với API mới của `AgentOrchestrator`**
  - Hiện `AgentOrchestrator.__init__` nhận `phobert_checkpoint` / `vit5_checkpoint` (không còn nhận `phobert_encoder` / `vit5_planner`).
  - Đã cập nhật `run_agent.py` thành một CLI đơn giản:
    - Khởi tạo:
      ```python
      AgentOrchestrator(
          max_steps=None,
    ...
    - Gọi:
      ...
      await orchestrator.execute_task(
          query="Mua áo khoác nam dưới 500k",
          start_url="https://example.com",
      )
      ```
    - In ra `success`, `steps`, `execution_time`, `error` để debug nhanh.

- [x] **Kích hoạt lại các test quan trọng**
  - Đã cài `pytest` + `pytest-asyncio` vào `venv`.
  - Đã thêm `path_setup.py` ở project root để các test có thể `import path_setup` mà không lỗi.
  - Đã sửa và chạy các test:
    - `tests/unit/test_perception.py`: dùng `@pytest.mark.asyncio`, test browser + `DOMDistiller` (Lazada).
    - `tests/unit/test_execution.py`: dùng `@pytest.mark.asyncio`, test `goto` / `type` / `click` (Google).
    - `tests/unit/test_planning.py`: hiện chỉ là placeholder, không lỗi.
  - Mức độ hoàn thành: pipeline cơ bản chạy được, không crash; test end-to-end chi tiết sẽ làm qua `tests/full_pipeline_test.py` ở giai đoạn sau.
...
- [ ] **Thiết kế action DSL chuẩn (NAVIGATE / CLICK / FILL / PRESS / SELECT_DROPDOWN / WAIT_FOR / CLICK_AT)**
  - Định nghĩa schema JSON cho 1 action:
    ...
    ```
  - Xây `Executor` (hoặc map trong `SkillExecutor`) từ `action_id` sang Playwright:
...
- [ ] **Observer + `page_state` + `available_actions`**
  - Viết module Observer riêng (có thể dựa trên `SceneRepresentation`, `UIDetector`, `DOMDistiller`, `VisionEnhancer`) để build:
    ...
    - `dom_state` với `products`, `filters`, `cart`, `checkout`.
    - `vision_state` (nếu bật vision).
    - `available_actions`: danh sách action DSL theo spec.
  - Ưu tiên implement cho 1 sàn (Shopee) trước, sau đó nhân bản rule cho Lazada.

- [ ] **Planner mới (LLM-1, sinh bước kế hoạch cấp cao)**
  - Định nghĩa interface Planner:
    - Input: `{ goal, high_level_state, history_summary }`.
    - Output: 
      ```json
      { "next_plan_step": { "step_id": "...", ... } }
      ```
      với các `step_id` hợp lệ: `SEARCH_PRODUCT`, `APPLY_FILTER`, `SELECT_PRODUCT`, `GO_TO_CART`, `GO_TO_CHECKOUT`, `FILL_CHECKOUT_INFO`, `REVIEW_ORDER`, `TERMINATE`.
  - Viết prompt template đầy đủ (system + few-shot), output JSON thuần.
  - Giai đoạn đầu: có thể dùng ViT5 hoặc model nhỏ hơn / state machine rule-based cho 2 sàn.
...
- [ ] **Controller mới (LLM-2, chọn action cụ thể)**
  - Định nghĩa interface Controller:
    ...
    - Output:
      ...
      ```
      hoặc action đặc biệt `CLICK_AT(bbox)` fallback.
  - Xây prompt system cho Controller:
    - Chỉ được chọn 1 action trong `available_actions`.
    - Không được click nút đặt hàng cuối cùng.

- [ ] **Tích hợp Planner + Controller vào main loop**
  - Cập nhật `AgentOrchestrator.execute_task`:
    - Thay vì để `ReActEngine` sinh trực tiếp skill, dùng flow:
      1. Observer → `page_state`.
      2. Planner → `next_plan_step`.
      3. Controller → `chosen_action`.
      4. Executor → thực thi action, sau đó quay lại Observer.
  - Giai đoạn chuyển tiếp: có thể giữ ReAct cho các task generic, dùng Planner/Controller mới cho task shopping.
...
- [ ] **Xác nhận với người dùng ở các bước quan trọng (UI/UX)**
  - Thiết kế một lớp `UserInteraction` (hoặc callback) để:
    ...
    - Khi cần nhập thông tin nhạy cảm (địa chỉ nhận hàng, số điện thoại, email):
      - Agent đề xuất giá trị (parse từ history / config) nhưng **luôn hỏi lại user** để confirm / chỉnh sửa.
  - Ở phiên bản CLI:
  ...
  - Chuẩn bị interface để sau này gắn vào UI web (tách logic hỏi/confirm khỏi core).

- [ ] **Nâng cấp `SafetyGuardrails` với rule “không tự đặt hàng”**
  ...
  - Kết hợp với `page_type == 'checkout'` hoặc plan step `REVIEW_ORDER`:
...
- [ ] **Siết domain whitelist cho TMDT**
  - Từ `SafetyGuardrails.get_allowed_domains()`:
    - Whitelist bắt buộc: `shopee.vn`, `lazada.vn` (và domain phụ nếu cần).
    - Các domain khác (`google.com`, `facebook.com`, `example.com`) chỉ dùng cho test/dev, có flag cấu hình riêng.
  - Trong orchestrator:
    - Nếu `start_url` không thuộc whitelist khi chạy ở chế độ “shopping”, từ chối và báo lỗi rõ ràng cho user.

- [ ] **Policy retry / timeout thân thiện**
  - Khi selector không tìm thấy hoặc trang load chậm:
    - Log rõ ràng (selector, URL hiện tại).
    - Hiển thị thông tin ngắn gọn cho user trên CLI.
...
- [ ] **Chuẩn hoá episode log**
  - Thiết kế cấu trúc episode:
    ```json
    {
      "episode_id": "ep_xxxx",
      "goal": "...",
...
- [ ] **Chuẩn bị pipeline fine-tune (Controller trước, Planner sau)**
  ...
  - Giai đoạn sau: cân nhắc fine-tune Planner hoặc chuyển Planner thành state machine riêng cho từng sàn.

---

## P4 - Model & offline inference

- [ ] **Tuỳ chọn backend LLM local**
  - Thiết kế abstraction cho LLM backend (thay vì gắn chặt vào ViT5):
    - Cho phép dùng: Qwen2 7B Instruct, Mistral 7B Instruct, LLaMA3 8B Instruct… thông qua:
      - Ollama, llama.cpp, vLLM (tuỳ môi trường).
  - Cấu hình qua `config/models.yaml` + `.env`:
    - Mode `"hf_online"` vs `"local_backend"`.

- [ ] **Lite mode / Controller nhẹ**
...
- [ ] **Nâng cấp UI/UX (tùy chọn, sau khi core ổn định)**
  - Thiết kế giao diện CLI “dễ dùng hơn”:
  ...
  - Về lâu dài: cân nhắc web dashboard đơn giản (FastAPI + frontend):
...
1) P0 → chạy được code, test unit cơ bản (đã xong).  
